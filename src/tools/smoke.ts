 ...
 *
 * 对契约 mock 对端全流程演示：注册表面 → 无预录调用 admit_data → blocked(approval_missing,
 * exit 78 锚点) → 账本预录 → admit_data 执行成功 → 账本一次性消费（重复调用 → blocked）→
 * gate advance（证据已准入 → pass）→ surface_scan / workspace_status（免审批只读）。
 *
 * 用法（仓库根目录）：
 *   npm run smoke:s3
 *
...
import { approvalKeyFor } from "./approvalKey.js";
...
const smoke = async (): Promise<void> => {
  ...
  try {
    ...
    console.log("[3] 账本预录 → admit_data 执行成功（canonical 校验通过）");
    const key = approvalKeyFor("atf_admit_data", params);
    const recorded = await connection.request("ledger_record", key);
    ...
    const gateKey = approvalKeyFor("atf_gate", gateParams);
    const gateRecorded = await connection.request("ledger_record", gateKey);
    ...
    console.log("[6] 只读工具免审批：surface_scan / workspace_status");
    const scan = await executor.execute("atf_surface_scan", {});
    step(
      "surface_scan count=1",
      scan.kind === "executed" && (scan.result as { count?: number }).count === 1,
    );
    const status = await executor.execute("atf_workspace_status", {});
    step(
      "workspace_status admitted_count=1",
      status.kind === "executed" && (status.result as { admitted_count?: number }).admitted_count === 1,
    );
  } finally {
